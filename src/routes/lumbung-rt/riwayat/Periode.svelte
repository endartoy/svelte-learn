<script>
	...
	import { getAllListPeriode, hardDelPeriode, restoreDelPeriode } from "$lib/supabase/tb_periode";
    ...
    let tb_periode = $derived(
        rawData.filter((res) => res.nama_periode.toUpperCase().includes(query.toUpperCase()))
    )
    const getData = () => {
        loadingStore.show()
        getAllListPeriode()
    ...
    const restoreData = () => {
        ...
        loadingStore.show()
        restoreDelPeriode(selected_data)
            .then((data) => {
        ...
        <div class="form">
            <div class="form-header">
                <span class="header-title"> 
                    { selected_data.id } - Periode { selected_data.nama_periode }
                </span>
            ...
            {:else}
                <button onclick={() => {disableBodyScroll(false); goto(`/lumbung-rt/periode/${selected_data.id}`)}} class="button primary">
                    <span>Edit Data</span>
